...
class VMEMapping(object):
    ...
    # to generate a bash script;
    def write_dump(self, offset, ctype, value):
        print "value = " + str(value)

        if ctype == ctypes.c_ubyte or ctype == ctypes.c_byte:
            q = 1
        if ctype == ctypes.c_ushort or ctype == ctypes.c_short:
            q = 1 
        if ctype == ctypes.c_uint or ctype == ctypes.c_int:
            q = 2
        if ctype == ctypes.c_ulonglong or ctype == ctypes.c_longlong:
            q = 4

        if value < 0:
            value = int(self.tohex(value,q*16), 16)
                        
        a = "{0:0{1}x}".format(offset,5)
        #v = str(hex(value)).strip('0x')
        v = "{0:0{1}x}".format(value,q*4)
        l = len(v) # must be multiple of 4
        n = 4 # num of digits in a chunk
        v_chunked = [v[l-i-n:l-i] for i in range(0,l,n)]
        cmd = '"writevme "$MasterSlot"' + a +" "+str(q)+" "+ ' '.join(list(reversed(v_chunked))) + '"'
        with open("test.sh", "a") as fd:
            #myfile.write('echo "'+cmd+'"\n')
            fd.write('cmd='+cmd+'\n')
            fd.write('echo $cmd\n')
            fd.write('$cmd\n')
            fd.write('\n')
    ...
    def write(self, offset, ctype, value):
        ...
        # to generate a bash script;
        self.write_dump(offset, ctype, value)
